...
- Replace `gloom` by `boom` in `limerick.txt` and run `stepup boot -j 1` again.
  The line numbering is repeated, but the step `./plan.py` is skipped as it did not change.

- Change the order of `static()` and `run()` in `plan.py` and run `stepup boot -j 1` again.
...
- Comment out the `static()` function call and run `stepup boot -j 1` again.
