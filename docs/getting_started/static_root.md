...
Example source files: [`docs/getting_started/static_root/`](https://github.com/reproducible-reporting/stepup-core/tree/main/docs/getting_started/static_root)

Create a subdirectory `data/` with one file: `somefile.txt`.
Also, create a `plan.py` file with the following contents:

```python
{% include 'getting_started/static_root/plan.py' %}
```
...
```text
{% include 'getting_started/static_root/stdout.txt' %}
```
