#!/usr/bin/env python3
"""Hold three children of increasing duration, declared in increasing (i.e. "wrong") order.

Without `hold()`, njob=2 leaves one job slot free once the plan step itself occupies the
other, so the first-declared child ("fast") would grab that free slot immediately, regardless
...
with hold():
    ...
    # A held child must stay ineligible for dispatch even though a job slot is free (njob=2:
