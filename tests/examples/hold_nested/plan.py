...
already-open outer `hold()` block. With `_holding` as a counter, children declared in either
...
prematurely, and all three children become eligible together only when the outer block closes.
"""
...
def declare_batch(prefix, durations):
    """Declare one batch of `work.py` children, each held until this call's block exits."""
    with hold():
...
with hold():
    ...
    # None of the three children -- from either nested batch -- may be dispatched while the
