...
rows = list(con.execute("SELECT seq, cmd, workdir, env, returncode FROM step_subprocess"))
con.close()

# Exactly one subprocess was recorded by the wrapper step.
assert len(rows) == 1, rows
seq, cmd, workdir, env, returncode = rows[0]
...
assert json.loads(env) == {"GREETING": "hello"}, env
