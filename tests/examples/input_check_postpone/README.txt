...
when both trigger on the same dispatch: a step that needs to be postponed
...
immediately, not postponed. main.sh asserts that desired outcome, so a
failing main.sh here documents a known gap in stepup/core/executor.py's
execute_job (wants_postpone is checked before run.success), not an
environment problem.
